...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, ispermanant, isfloating, canfocus, isurgent, neverfocus, oldstate, isfullscreen, issticky;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
{
	if(!selmon->sel || selmon->sel->ispermanant)
		return;
